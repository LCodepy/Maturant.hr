...
export const authOptions: NextAuthOptions = {
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        return {
          ...
          paidLectures: existingUser.paidLectures,
        };
      },
    }),
  ],
  callbacks: {
    ...
    async session({ session, token }) {
      ...
      // Fetches only paidLectures property from the user with given email
      const user = await db.user.findUnique({
        where: { email: session.user.email as string },
        select: { paidLectures: true },
      });

      // Add the user field with username and paidLectures fields because they are not there by default
      return {
        ...session,
        user: {
          ...session.user,
          username: token.username,
          paidLectures: user?.paidLectures,
        },
      };
    },
